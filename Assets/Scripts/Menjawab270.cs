...
public class Menjawab270 : MonoBehaviour {

	public Sprite BlackCircle3;
	public Sprite RedCircle3;
	public GameObject textBox3;
	public GameObject textBox23;
	public GameObject textBox33;
	public GameObject gift3;
	public GameObject laser3;
	public Text theText3;
	public Text userjawab3;
	public Text userjawab23;
	...
	void Start () {
		...
		textBox23.SetActive (false);
		textBox33.SetActive (false);
		gift3.SetActive (false);
		load = FindObjectOfType<LevelLoader> ();

		this.gameObject.GetComponent<SpriteRenderer> ().sprite = BlackCircle3;
	...
	IEnumerator DisplayTimer(){
		while (1 == 1) {
			...
			theText3.text = textLines [stringindex].Substring(0, characterIndex);
			characterIndex++;
		}
	}
	...
	void OnTriggerEnter2D(Collider2D other){
		if (other.name == "Player" && gift3.active==false) {
			playerposx = 0;
			playerposy = 0;
			stringindex = 0;
			textBox3.SetActive (true);
			this.gameObject.GetComponent<SpriteRenderer> ().sprite = RedCircle3;
			StartCoroutine (DisplayTimer ());
			playerposx = Mathf.Round(laser3.transform.position.x);
			playerposy = Mathf.Round(laser3.transform.position.y);
			float suduttttt=270;
	...
	void Update () {
		...
		if (stringindex == 7) {
			textBox23.SetActive (true);

			string jawaban = userjawab3.text.ToString ();
			if (jawaban == hasilsatu && Input.GetKeyDown (KeyCode.Return)) {

				textBox23.SetActive (false);
				textBox33.SetActive (true);
				userjawab3.text = "jawaban anda";


			} 
		}
		if (stringindex == 8) {
			string jawaban2 = userjawab23.text.ToString ();
			if (jawaban2 == hasildua && Input.GetKeyDown (KeyCode.Return)) {
				textBox33.SetActive (false);
				textBox23.SetActive (false);
				textBox3.SetActive (false);
				if (this.gameObject.GetComponent<SpriteRenderer> ().sprite == RedCircle3) {
