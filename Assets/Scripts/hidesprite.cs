...
public class hidesprite : MonoBehaviour {
	...
	void Start () {
		...
		hadiah.SetActive (false);
		//load = FindObjectOfType<LevelLoader> ();

		this.gameObject.GetComponent<SpriteRenderer> ().sprite = beforeimage;
	}
	IEnumerator DisplayTimer(){
		while (1 == 1) {
			...
			textpenjelasan.text = textLines [stringindex].Substring(0, characterIndex);
			characterIndex++;
		}
	}
	...
	void OnTriggerEnter2D(Collider2D other){
		if (other.name == "Player" && hadiah.active==false ) {
			playerposx = 0;
			playerposy = 0;
			stringindex = 0;
			panelpenjelasan.SetActive (true);
			this.gameObject.GetComponent<SpriteRenderer> ().sprite = afterimage;
			StartCoroutine (DisplayTimer ());
			playerposx = Mathf.Round(posisilaser.transform.position.x);
			playerposy = Mathf.Round(posisilaser.transform.position.y);
			float suduttttt=90;
	...
	void Update () {
		...
		if (stringindex == 9) {
			panelpenjelasan.SetActive (false);
			if (this.gameObject.GetComponent<SpriteRenderer> ().sprite == afterimage) {
